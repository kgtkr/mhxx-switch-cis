package com.github.kgtkr.mhxxSwitchCIS

import java.awt.*

object ImageConfig{
    const val IMAGE_W=1280
    const val IMAGE_H=720

    const val TABLE_X=862
    const val TABLE_Y=99
    const val TABLE_W=398
    const val TABLE_H=288

    const val ROW_H=32
    const val ROW_COUNT=9

    const val GOSEKI_X=0
    const val GOSEKI_W=31

    const val ONE_SKILL_NAME_X=31
    const val ONE_SKILL_NAME_W=112

    const val ONE_SKILL_VALUE_X=147
    const val ONE_SKILL_VALUE_W=36

    const val TWO_SKILL_NAME_X=182
    const val TWO_SKILL_NAME_W=112

    const val TWO_SKILL_VALUE_X=297
    const val TWO_SKILL_VALUE_W=36

    const val SKILL_VALUE_NUMBER_W=12

    const val SLOT_X=331
    const val SLOT_W=62

    //色を取得する座標
    const val SLOT_COLOR_Y=16
    const val SLOT1_COLOR_X=13
    const val SLOT2_COLOR_X=31
    const val SLOT3_COLOR_X=48

    const val GOSEKI_COLOR_X=10
    const val GOSEKI_COLOR_Y=24

    val COLOR=Color(225,225,225)
    const val THRESHOLD=10000

    val SLOT_COLOR=Color(255,255,255)
    const val SLOT_THRESHOLD=30000

}